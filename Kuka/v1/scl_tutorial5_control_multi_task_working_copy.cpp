...
{ 
  ...
  bool flag = p.readRobotFromFile("./KukaCfg.xml","./","KukaBot",rds);
  ...
  flag = p.readTaskControllerFromFile("./KukaCfg.xml","opc",must_use_robot,rtasks,rtasks_nc);
  if(must_use_robot != "KukaBot") {flag = false;} //Error check for file consistency
  ...
  flag = p.readGraphicsFromFile("./KukaCfg.xml","KukaBotStdView",rgr);
...
#pragma omp parallel private(thread_id)
  {
	...
	if(impact){
	ball_force=(nomral_velctor*ball_vell)*ball_vel.norm()
        ball_force=-gravity*(0,0,1) - .5*rho*pi*r*r*Cd*(ball_velocity.norm()^2)


        // Move the hand in a sine wave
        rtask_hand->x_goal_(0) = x_cross(0); //crossing location in z plane
        rtask_hand->x_goal_(1) = y_cross(1);
